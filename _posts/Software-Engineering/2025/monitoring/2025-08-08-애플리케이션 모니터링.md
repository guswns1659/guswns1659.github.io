...
<img src="https://github.com/user-attachments/assets/ea87ddf1-9797-4a83-8871-d5e59bd997ee" width="948" height="564" >
...
<img src="https://github.com/user-attachments/assets/4ea7e04e-fcac-47ed-a0be-f8672e47d988" width="701" height="546">
...
![img_1.png](https://github.com/user-attachments/assets/d494cce0-5dfb-441b-9dc7-644cddcf2733) 
...
1. **Spring Boot Actuator와 Micrometer**
  ...
  * Micrometer:
    * 계측(Instrumentation) 라이브러리입니다. 즉, '어떻게' 메트릭을 측정하고 수집할지에 대한 표준 API(Counter, Timer, Gauge 등)를 제공합니다.
    * 다양한 모니터링 시스템(Prometheus, New Relic, Datadog 등)에 맞는 포맷으로 데이터를 변환하는 MeterRegistry 구현체들을 제공합니다.
    * SLF4J가 로깅의 '추상화 라이브러리'인 것처럼, Micrometer는 메트릭의 '추상화 라이브러리' 역할을 합니다.

![img_5.png](https://github.com/user-attachments/assets/e0b45919-93e2-4069-9917-98301dac442e)

(출처: [https://micrometer.io/](https://micrometer.io/))


  * Spring Boot Actuator:
    * 통합 및 노출(Integration & Exposure) 계층입니다.
    ...
    * Micrometer를 Spring Boot 환경에 자동으로 구성(Auto-configuration)하고, 필요한 Bean(MeterRegistry, MeterBinder 등)을 생성해 줍니다.
    * Micrometer가 수집한 메트릭을 /actuator/metrics, /actuator/prometheus 같은 HTTP 엔드포인트를 통해 외부에서 조회할 수 있도록 노출하는 역할을 담당합니다


2. **내부 동작 원리**

   * **애플리케이션 내부**: Timer, Gauge 같은 Meter가 JVM, HTTP, DB 등 다양한 메트릭을 수집하고 `PrometheusMeterRegistry`에 등록합니다. Actuator는 `/actuator/prometheus` 엔드포인트를 통해 이 데이터를 외부에 노출합니다.
![img_11](https://github.com/user-attachments/assets/5447e697-cbb6-426e-9434-9fb05b993e12)
...
![img_4.png](https://github.com/user-attachments/assets/52e8f3f2-5e74-49a3-8d19-9ad317b91b17)
...
**Prometheus** 같은 TSDB는 수집한 데이터를 먼저 **WAL(Write Ahead Log)**에 기록하고, 일정 기간(예: 2시간) 단위로 압축 블록에 저장합니다.
...
![img_3.png](https://github.com/user-attachments/assets/7f01ee05-2708-4bf1-94d2-f0c12e9de3a7)
