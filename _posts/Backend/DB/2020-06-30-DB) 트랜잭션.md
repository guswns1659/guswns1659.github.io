...
- ACID란 무엇인가? 각 약자에 대해 설명하라
  - Atomicity : 각 트랜잭션은 하나의 원자처럼 성공하거나 도중에 실패하면 다시 돌아간다.
  - consistency : 트랜잭션 이후 데이터의 무결성(제약조건)이 유지되는 정도.
  - Isolation : 동시에 실행돼도 마치 한 개의 트랜잭션만 사용하는 것처럼 유지되는 정도
  - Durability : 커밋되면 데이터가 영구적으로 보존되는 속성
